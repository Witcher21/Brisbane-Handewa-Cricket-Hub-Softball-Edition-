  ...
  <q-page class="dark-page">
    <div class="page-container">
      ...
      <div class="adm-header">
        ...
        <div class="adm-links">
          <router-link to="/broadcast" class="overlay-btn" target="_blank"
            >📺 OBS Overlay</router-link
          >
          <router-link v-if="match" :to="`/matches/${match.id}`" class="overlay-btn"
      ...
      <div v-if="!match" class="no-match-state">
        ...
        <p>
          Go to <router-link to="/manage">Manage → Matches</router-link>, then click
      ...
      <template v-else>
        ...
        <div class="score-banner">
          ...
          <div class="sb-center">
            ...
            <div class="sb-crr">
              CRR: {{ crr
              }}<span v-if="inning.target">
                · Need {{ inning.target - inning.runs }} off
                {{ match.overs - Math.floor((inning.legalBalls || 0) / 6) }} ov</span
              >
            </div>
            <!-- Overs progress bar -->
        ...
        <div v-if="showInnings2Btn" class="innings2-banner">
          <div class="i2b-text">
            ...
            <span v-if="match.innings[0]"
              >Score: {{ match.innings[0].runs }}/{{ match.innings[0].wickets }}</span
            >
          </div>
          <button
            v-if="match.currentInning !== 2"
            class="i2b-btn"
            @click="store.startInnings2(match.id)"
          >
            Start 2nd Innings →
          </button>
        </div>
        ...
        <div class="over-bar">
          <span class="ob-label"
            >Over {{ completedOvers + 1 }} ({{ inning.currentOverBalls || 0 }}/6):</span
          >
          <div class="ob-balls">
            <span v-for="(b, i) in thisOverBalls" :key="i" class="ball" :class="ballClass(b)">{{
              b
            }}</span>
        ...
        <div class="crease-panel">
          ...
          <div class="batter-card striker-card" v-if="striker">
            ...
            <div class="bc-extras">
              {{ striker.fours }}×4 · {{ striker.sixes }}×6 · SR
          ...
          <div class="batter-card nonstriker-card" v-if="nonStriker">
            ...
            <div class="bc-body">
              <div class="bc-name ns-name">
                {{ store.getPlayerById(nonStriker.playerId)?.name }}
              </div>
          ...
          <div class="crease-footer">
            <button
              class="swap-btn"
              @click="store.swapStriker(match.id)"
              title="Manually swap striker"
            >
              ⇄ Swap Strike
            </button>
        ...
        <div class="bowl-panel">
          ...
          <div class="bowl-select-row">
            ...
            <button
              v-for="p in fieldingTeamPlayers"
              :key="p.id"
              class="bowl-pick"
        ...
        <div class="bo-panel">
          ...
          <div class="bo-scroll">
            <div class="bo-head">
              ...
              <span class="bon">R</span><span class="bon">B</span> <span class="bon">4s</span
              ><span class="bon">6s</span>
            ...
            >
              ...
              <span class="bon sr-col">{{
                b.status === 'yet' ? '-' : b.balls > 0 ? ((b.runs / b.balls) * 100).toFixed(0) : 0
              }}</span>
              <span class="bon-stat">
        ...
        <div class="log-panel">
          ...
          <div class="log-scroll">
            <div v-for="n in matchCommentary" :key="n.id" class="log-row" :class="`lt-${n.type}`">
...
// ✅ Always use the LATEST innings (works for both 1st and 2nd innings)
const inning = computed(() => {
  if (!match.value?.innings?.length)
    return { runs: 0, wickets: 0, overs: 0, legalBalls: 0, currentOverBalls: 0 }
  return match.value.innings[match.value.innings.length - 1]
})
...
// ✅ Bowling team flips with innings: Team B bowls in 1st, Team A bowls in 2nd
const fieldingTeamPlayers = computed(() => {
  if (!match.value) return []
  const fieldingTeamId = match.value.currentInning === 2 ? match.value.teamAId : match.value.teamBId
  return store.getPlayersByTeam(fieldingTeamId)
})
...
const showInnings2Btn = computed(() => {
  ...
  const maxOv = match.value.overs || 20
  const inn1Done =
    completedOvers.value >= maxOv || (inning.value.wickets >= 10 && inning.value.inningsNum === 1)
  return inn1Done || match.value.currentInning === 2
})

// Persistent commentary log filtered to this match only
const matchCommentary = computed(() => {
  if (!match.value) return []
  return (store.commentaryLog || []).filter((c) => c.matchId === match.value.id).slice(0, 80)
})
...
.ball-wkt {
  background: #ff3d57;
  color: #fff;
  box-shadow: 0 0 10px rgba(255, 61, 87, 0.5);
}
.ball-six {
  background: #a855f7;
  color: #fff;
...
.bp-active {
  ...
  border-color: rgba(255, 61, 87, 0.3) !important;
...
.wkt-btn {
  ...
  transition: all 0.18s;
  font-family: inherit;
  box-shadow: 0 4px 18px rgba(255, 61, 87, 0.35);
  letter-spacing: 1px;
}
.wkt-btn:hover {
  transform: translateY(-3px);
  box-shadow: 0 8px 28px rgba(255, 61, 87, 0.55);
}
...
.bo-row {
  display: grid;
  grid-template-columns: 1fr 36px 36px 32px 32px 42px 110px;
  gap: 4px;
  align-items: center;
  min-width: 480px;
}
